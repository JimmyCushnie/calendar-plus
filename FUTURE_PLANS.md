...
- **2.1.8 (feature-image lag, the big one):** three layers fixed. (1) **note switching** no longer ticks the calendar — `updateActiveFile()` just sets the active-file stores and the highlight moves reactively, so switching notes no longer recomputes all 42 cells. (2) **per-cell rendering is in place** — `MetadataResolver` resolves into `$state` + `{@render}` instead of `{#await}`, so a new metadata promise per tick no longer destroys/recreates each image cell's DOM (the DOM churn forced a full image re-decode/re-composite every tick — this was the typing/sync lag). (3) **feature images render from cached downscaled thumbnails** (`src/io/thumbnailCache.ts`, plugin-folder files, generated once via `createImageBitmap` → canvas → JPEG, bounded concurrency, mark-and-sweep GC) instead of full-res `app://` URLs — this removed the sidebar-open decode stutter and bounds cost on image-heavy vaults / phones.
- **2.1.9:** (1) **ticks are coalesced** — the imperative `tick()` is `debounce`d (~16ms) in `src/ui/Calendar.svelte`, so file-event fan-out (double-tick on frontmatter edits; one tick per file during a sync burst) collapses to a single `today` bump; a tick is idempotent so this only removes redundant recompute. (2) **thumbnail cold-open memory** is bounded — `readBinary` moved inside the concurrency gate, so ≤3 full-res buffers are held at once instead of ~30. (3) the **`word-count-tasks` `cachedRead` was memoized** (see below — this thread is now done).
...
- **DONE (2.1.9): `word-count-tasks` `cachedRead` per cell per tick.** The `wordCount` and `tasks` sources now share `getContentMetrics` (`src/ui/sources/contentMetrics.ts`) — one `cachedRead` + scan per note, memoized by `(path, mtime)` and Promise-cached so overlapping ticks dedupe — instead of two independent reads+scans per visible cell per tick. Combined with tick coalescing, that mode is now responsive while typing. Kept self-gated, so zero cost in the default `exists` mode.
- **The `today`-bump-as-universal-refresh design.** Every refresh (settings change, file create/delete/modify/rename, metadata change, active-file sync) works by reassigning `today`, which re-evaluates `getDailyMetadata(...)` for *all* visible cells (the metadata expressions read the note stores via `get()`, not reactively, so a blanket trigger is needed). A more granular model — making cells reactively depend on their own store entry so only changed cells recompute — would cut recompute scope, but it's a meaningful rearchitecture (touches the source/metadata layer). **2.1.9's tick coalescing captured most of the practical benefit** (fewer, cheaper ticks) at a fraction of the risk, so this rearchitecture is lower priority now. Don't attempt without profiling justifying it.
...
## Deferred from the Fable review (Jul 2026) — ready to pick up

A code review (Fable model) of the 2.1.11 diff + vendored components, plus a design and a performance pass, produced these. The correctness regression it found (`removeByOldPath` guarding on `.path` instead of file identity) was fixed in 2.1.12, along with two perf wins (`monthsWithNotes` gated on `showYearOverview` + ISO-substring parse). The items below were deliberately deferred — none are bugs; they're perf polish and robustness/maintainability. Listed most-worth-doing first so they can be picked up as a batch.

- **Settings deep-merge + typed nested writer (robustness — highest leverage).** `loadOptions` (`src/main.ts`) hand-enumerates a one-level merge per nested settings object, and `writeOptions` is a top-level `{...old, ...change}` that relies on every nested writer to spread `prev`. Under the no-migration policy this default-backfill is the *entire* forward-compat mechanism, and it fails silently: a future two-levels-deep setting won't backfill, and one writer that forgets to spread `prev` drops sibling fields. Fix: a generic `deepMergeDefaults(defaults, saved)` in `loadOptions` + a typed nested-write helper. Low risk, well-contained. **Would do.**
- **`localeData` stabilization (perf — P2 from the perf pass).** `Calendar.svelte` passes `localeData={{...today.localeData()}}` — a new object identity every tick — so the vendored `Calendar.svelte`'s `month`/`daysOfWeek` `$derived` (42 moment clones + 6 `.week()`) rebuild on every content tick, though grid geometry only depends on `displayedMonth` + locale/weekStart. Fix: derive `localeData` from the locale inputs (`$settings.localeOverride`/`weekStart`) instead of `today`, so it's stable across content ticks but fresh on locale change. **Real (small) regression risk** — get the dependency wrong and the grid won't re-rotate when "Start week on" / locale override changes, so it **must be load-tested** (flip weekStart + locale → header/grid update). Worth doing; adds test burden.
- **Hoist the emoji-tag regex (perf — P3, do carefully).** `src/ui/sources/tags.ts` rebuilds a ~40-branch surrogate-pair regex literal on every `.test()` call (per tag, per cell with a note, per tick). Hoisting it to a module-level `const` (no `/g`, stateless) is a one-line win — but the regex is written with literal `\uXXXX` escapes and any transcription error silently breaks emoji-tag classification, so edit it by moving the exact existing bytes (not retyping) and verify tag/emoji-tag detection still works. Deferred from 2.1.12 precisely because a hotfix release wasn't the place to risk it. Negligible impact; low priority.
- **`view.ts` `openOrCreate*` de-duplication (maintainability).** The five per-period `openOrCreate{Daily,Weekly,Monthly,Quarterly,Yearly}Note` methods are near-identical (periodicity literal + `startOf` unit), and `view.ts` (~850 lines) is a god-object absorbing file events, menu construction, feature-image toggles, and thumbnail wiring. New features fork more of these (second-daily already did). Fix: a table-driven `openOrCreatePeriodic(periodicity, date, ctrl)` keyed on a small config map, keeping the second-daily special case separate. Moderate, low-risk, mechanical — **do it when next substantially touching `view.ts`**, not standalone.
- **Async ceremony for synchronous sources (perf/architecture — watch, profile first).** ~5 of 6 sources (`exists`/streak/feature-image/second-daily/tags) resolve synchronously from stores + metadataCache, yet the `getDailyMetadata → metadataReducer → Promise.all` contract wraps them all in promises and forces the per-cell `MetadataResolver` `$effect` unwrap every tick. Allowing `IDayMetadata | Promise<IDayMetadata>` and a sync path in `MetadataResolver` would remove the per-cell allocation/microtask in the common case — but it touches the core `ICalendarSource` contract (real regression surface across the render layer). Distinct from the deferred `today`-bump rearchitecture (that's *scope* of recompute; this is *per-cell overhead* when they do). Only act on devtools evidence; tick coalescing already blunted the frequency.
- **`getDateUID` recomputed ~7×/cell/tick (perf — watch).** `Day.svelte` computes the daily UID once and each of the 6 sources recomputes it via `getPeriodicNote` → `getDateUID` (moment `clone().startOf().format()`) for the same date. A `WeakMap<Moment,string>` memo per periodicity would collapse the same-tick calls (a fresh Moment per tick means it self-evicts). Marginal; only worth it if `format()` shows hot in a profile.
- **Path-keyed memos don't evict on delete (noted, harmless).** `getContentMetrics` and `resolveFeatureImageFile` leave one stale entry per deleted note; bounded by distinct notes seen in a session, self-invalidating on mtime. Not worth eviction logic.
- **Module-level singletons couple module lifetime to plugin lifetime (watch).** `stores.ts` writables, `thumbnailCache.ts`'s `app`/`urlByKey`/`epoch`, and `sweptThisSession` in the feature-image source assume a single plugin instance. Fine today (Obsidian re-evaluates the bundle on re-enable), and `sweptThisSession` only resets on bundle re-eval. The ready-callback already shifted single-slot → `Set` for multi-view; if multiple calendar views ever land (already flagged for `displayedMonth`), audit these globals as a *set*, not one at a time.

Design choices the review explicitly judged **sound — do not "improve"**: the thumbnail-cache subsystem (its complexity is load-bearing — each part answers a measured problem), the `sources/` self-gating + factory pattern, `TRIGGER_ON_OPEN` injection, the reactive `activeFile` highlight decoupled from `tick()`, and settings-as-the-reactive-spine.
