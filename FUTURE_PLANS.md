...
A code review (Fable model) of the 2.1.11 diff + vendored components, plus a design and a performance pass, produced these. The correctness regression it found (`removeByOldPath` guarding on `.path` instead of file identity) was fixed in 2.1.12, along with two perf wins (`monthsWithNotes` gated on `showYearOverview` + ISO-substring parse). **The top two robustness/perf items were then done in 2.1.13** (settings deep-merge; `localeData` stabilization). The items still below were deliberately deferred — none are bugs; they're perf polish and maintainability. Listed most-worth-doing first so they can be picked up as a batch.

- **DONE (2.1.13): Settings deep-merge + nested writer.** `loadOptions`/`writeOptions` (`src/main.ts`) now use one generic `deepMerge` — defaults backfill at any depth on load, and a `writeOptions` caller returning a nested partial can't drop sibling fields. Replaced the hand-enumerated one-level load merge + shallow top-level write spread. Behavior-preserving for the current shape (verified against all 25 callers); arrays replace wholesale, `undefined` patch values ignored.
- **DONE (2.1.13): `localeData` stabilization (perf — was P2).** `Calendar.svelte` now keys `localeData` on the locale settings (`localeOverride`/`weekStart`) via `moment()` rather than spreading `{...today.localeData()}` every tick, and the vendored `Calendar.svelte`'s `daysOfWeek` derived was retargeted from `today` to `localeData`, so `getMonth`/`getDaysOfWeek` stop rebuilding on every content tick. Preserves re-rotation on weekStart/locale change (the trigger fires on those settings).
